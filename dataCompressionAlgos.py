...
def main():
    ...
    # String given in slides
    text_binary = "0" * 12 + "1" + "0" * 3 + "11" + "0" * 8
    print("text_binary ", text_binary)
    print("RLE v2 of example: ", dc_run_length_encoding_v2(text_binary))

    text = "BAABABBBAABBBBAA"
    alphabet = ["A", "B"]
    print("LZW Example: ", dc_lempel_ziv_welch(alphabet, text))

    alphabet = ["A", "B", "C", "D", "E"]
    freqs = [20, 10, 10, 30, 30]
    print("Huffman coding example: ", dc_huffman_coding(alphabet, freqs))

    # see figure 8.7 on slide 8.2
    text = "BBAB"
    alphabet = ["A", "B", "*"]
    freqs = [.4, .5, .1]
    print("Arithmetic encoding: ", dc_arithmetic_encoding(text, alphabet, freqs))
